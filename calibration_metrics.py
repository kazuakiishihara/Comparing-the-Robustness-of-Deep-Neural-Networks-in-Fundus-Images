...
class CalibrationMetric():
    ...
    def _compute_error_all_binned(self, binned_fx, binned_y, bin_sizes):
        self.bin_conf = binned_fx.squeeze()
        self.bin_acc = binned_y.squeeze()
    ...
    def _calc_ece_postbin(self, n_bins, bins, fx, y):
        ...
        for i in range(n_bins):
            cur = bins == i
            if any(cur):
                # self.bin_max[i] = np.max(fx[cur])
                self.bin_conf[i] = fxm = np.mean(fx[cur])
                self.bin_acc[i] = ym = np.mean(y[cur])
